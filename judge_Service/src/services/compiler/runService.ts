...
import { executeRunCode } from "./executeRunCode";

export async function runService(code: string, testcases: any, language: string, timeLimit: number, memoryLimit: number) {
    ...
    let output = await executeRunCode(path, testcases, language, timeLimit, memoryLimit);
    let verdict
    let result: any = [];
